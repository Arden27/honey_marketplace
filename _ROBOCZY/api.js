...
export const registerUser = (username, password1, password2) => {
  return fetchApi("/dj-rest-auth/registration/", "POST", {
    username,
    password1,
    password2,
  }).catch((err) => {
...
export const loginUser = (username, password) => {
  return fetchApi("/dj-rest-auth/login/", "POST", { username, password }).catch(
    (err) => {
      console.error(err);
      return Promise.reject(err); // propagate the error up to the component
    },
...
export const logoutUser = () => {
  return fetchApi("/dj-rest-auth/logout/", "POST").catch((err) => {
    console.error(err);
    return Promise.reject(err); // propagate the error up to the component
  });
};
